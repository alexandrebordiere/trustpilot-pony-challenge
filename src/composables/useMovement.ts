...
export default function useMovement (maze: Maze): UseMovement {
  ...
  const canMoveToCellBelow = (position: number) => !isLastRow(position) && !maze.structure[getAdjacentCellPosition(position, 'south')].includes('north');
  const canMoveToNextCell = (position: number) => !isLastColumn(position) && !maze.structure[getAdjacentCellPosition(position, 'east')].includes('west');
  const canMoveToPreviousCell = (position: number) => !isFirstColumn(position) && !maze.structure[position].includes('west');
  ...
  const getPossibleMovements = (position: number): Movement[] => {
    ...
    return directions.map(direction => ({ direction, newPosition: getAdjacentCellPosition(position, direction) }));
  };
